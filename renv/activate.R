...
local({
  ...
  if (diagnostics) {
    ...
    on.exit({
      utils::Rprof(NULL)
      elapsed <- signif(difftime(Sys.time(), start, units = "auto"), 
                       digits = 2L)
      writeLines(sprintf("- renv took %s to run the autoloader.", 
                        format(elapsed)))
  ...
  if (!enabled) {
    ...
    # If not enabled, manually load user profile here
  ...
  # Signal for loading renv during R startup
  options(renv.autoloader.running = TRUE)
  on.exit(options(renv.autoloader.running = NULL), add = TRUE)

  # Signal for consented use of renv
